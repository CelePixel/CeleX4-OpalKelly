...
using namespace std;
cv::VideoWriter cvVideoWriterEventPic;
    ...
    , m_bCreatingFramePaused(false)
{
    ...
    cvVideoWriterEventPic.open("video.avi", CV_FOURCC('X', 'V', 'I', 'D'), 20.0, cv::Size(768, 640), true);
...
void SensorDataObserver::updateImage(unsigned char *pBuffer1, unsigned char *pBuffer2)
{
    ...
    if (m_bSaveBmps)
    {
        ...
        uchar* pRaw = m_imageFullPic.bits();
        ...
        cvVideoWriterEventPic.write(matOpticalRawColor);
    }
...
std::time_t MainWindow::getTimeStamp()
{
...
void MainWindow::playback()
{
    ...
    if(button[0]->text()=="Playback")
    {
        ...
        m_pCelexSensor->setPlaybackState(CeleX4::Playing);
    ...
    {
        displayPlaybackBoard(false);
        button[0]->setText("Playback");
        pButton->setStyleSheet("QPushButton {background: #002F6F; color: white; "
                               "border-style: outset; border-width: 2px; border-radius: 10px; border-color: #002F6F; "
                               "font: 20px Calibri; }"
                               "QPushButton:pressed {background: #992F6F;}");
        m_pCelexSensor->setPlaybackState(CeleX4::NoBinPlaying);
    }
}
...
void MainWindow::onDisplayMode1Changed(int mode)
{
    ...
    {
        if (m_uiDisplayMode2== 8 || m_uiDisplayMode2 < 5)
...
void MainWindow::onDisplayMode2Changed(int mode)
{
    ...
    {
        if (m_uiDisplayMode1 == 8 || m_uiDisplayMode1 < 5)
            m_pCelexSensor->enableOpticalFlow(false);
    }
    if (mode == 8)
    {
